def randomforest(windowsize,crossvalnum):
	...
	for i in range(0,len(protname_test)):
		...
		for j in range (0,len(protname_test[i])):
			...
			for k in range(0,zero):
				pssmlist.append(list(np.zeros(20)))
			...
			for k in range(3, len(pssmfile)-6):
				...
				seq = seq.split()
				seq = seq[2:22] # substitution matrix
				#seq = seq[22:42] # frequency matrix



				# make sure that all the numbers are converted to float and normalized by iterating through the list
				for l in range (0,len(seq)):
					seq[l] = float(sigmoid(float(seq[l])))
				pssmlist.append(seq)
			for k in range(0,zero):
				pssmlist.append(list(np.zeros(20)))
			for k in range(zero,len(pssmlist)-zero):
				if k+zero > len(pssmlist):
					break
				#for k in range(i-(zero),i+1+(zero)):
				#	element = pssmlist[k]
				#	lst.extend(element)
				#windows.append(lst)
				#lst = []		
				tmp = pssmlist[k-zero:k+zero+1]	
				tmp = [j for k in tmp for j in k]
				tmplist.append(tmp)
	...
	for i in range(0,len(windows_train)):
		...
		clf.fit(windows_train[i], ss_train_new[i])
		prediction = clf.predict(windows_test[i])
	...
	printfile = open('windowsize_' + str(win) + '_fold_validation_' + str(cvnum) + '_SM_randomforest.txt', 'w+')
	...
	plt.title('windowsize: ' + str(win) + ' fold-validation: ' + str(cvnum) + ' F1: ' + str(F1) + '_SM_RF')
	plt.colorbar()
	...
	plt.savefig('windowsize_' + str(win) + 'fold_validation_' + str(cvnum) + '_SM_randomforest.png')
	#plt.show()

randomforest(windowsize = 7, crossvalnum = 5)
randomforest(windowsize = 9, crossvalnum = 5)
randomforest(windowsize = 11, crossvalnum = 5)
randomforest(windowsize = 13, crossvalnum = 5)
randomforest(windowsize = 15, crossvalnum = 5)
randomforest(windowsize = 17, crossvalnum = 5)
